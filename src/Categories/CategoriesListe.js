...
function CategoriesListe(props) {
  ...
  for(let i = currentItems ; i < (currentItems+12) ; i++){
    list.push(props.categories[i] ? (<Categorie key={props.categories[i].id} id={props.categories[i].id}  nom={props.categories[i].nom} onChange={handleCategorieChange} />) : (<div  key={i} className='emptyCategorie mb-10'></div>))
  }
